...
bars_tf = (DAYS * 24 * 60) // 30 + 300
print(f"Mengambil data {SYMBOL} {DAYS} Hari...")
rates_tf = mt5.get_bulk_rates(SYMBOL, 30, bars_tf)
...
dx = 100 * abs(df_tf['+di'] - df_tf['-di']) / (df_tf['+di'] + df_tf['-di'])
df_tf['adx'] = dx.ewm(alpha=1/period, adjust=False).mean()

df_tf['prev_+di'] = df_tf['+di'].shift(1)
df_tf['prev_-di'] = df_tf['-di'].shift(1)
df_tf['prev_adx'] = df_tf['adx'].shift(1)
...
print("=========================================")
print(f"Data M30: {len(df_tf)} bars")
...
for i in range(1, len(df_tf)):
    ...
    for p in active_pos:
        ...
        if closed:
            ...
            trades.append(p)
            closed_pos.append(p)
            print(f"[{curr['time']}] CLOSE {p['type']} at {curr['close']:.5f} | Reason: {p['reason']:<5} | PnL: ${pnl:.2f}")
    ...
    if len(active_pos) < MAX_POSITIONS:
        ...
        if buy_signal or sell_signal:
            ...
            if buy_signal:
                ...
                typ = 'BUY'
                print(f"[{curr['time']}] OPEN BUY  | Price: {entry_price:.5f} | SL: {sl:.5f} | TP: {tp:.5f} | ATR: {entry_atr:.5f} | ADX: {curr['adx']:.2f}")
            else:
                entry_price = curr['open'] - spread
                sl = entry_price + (entry_atr * SL_MULT)
                tp = entry_price - (entry_atr * TP_MULT)
                typ = 'SELL'
                print(f"[{curr['time']}] OPEN SELL | Price: {entry_price:.5f} | SL: {sl:.5f} | TP: {tp:.5f} | ATR: {entry_atr:.5f} | ADX: {curr['adx']:.2f}")
...
print("\n=========================================")
print(f"HASIL BACKTEST ADX DOMINANCE {SYMBOL}")
print("=========================================")
...
if trades:
    ...
    final_count = sum(1 for t in trades if t['reason'] == 'FINAL')
    
    print(f"Win Rate     : {win_rate:.1f}% ({len(wins)}W / {len(losses)}L)")
    print(f"--- Rincian Exit ---")
    print(f"✅ Kena TP (Target 4.0 ATR) : {tp_count} trades")
    print(f"❌ Kena SL (Loss 2.5 ATR)   : {sl_count} trades")
    print(f"🛡️ Kena BE (Break Even)     : {be_count} trades")
    print(f"⏳ Posisi Akhir (FINAL)     : {final_count} trades")
    print(f"--------------------")
